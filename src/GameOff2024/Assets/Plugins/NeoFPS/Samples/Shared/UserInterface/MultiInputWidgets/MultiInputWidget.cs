    ...
    public abstract class MultiInputWidget : Selectable
	{
        ...
        public int indentLevel
        {
			...
			set
            {
				...
				if (m_Indentable != null)
                {
					var anchoredPos = m_Indentable.anchoredPosition;
					anchoredPos.x = value * indentSize;
					m_Indentable.anchoredPosition = anchoredPos;

					float widthDiff = oldIndentLevel * indentSize - m_IndentLevel * indentSize;

					//var sizeDelta = 
					m_Indentable.offsetMax = new Vector2(spacing * -0.5f, 0f);
				}
			}
        }
        ...
        public override void OnMove (AxisEventData eventData)
		{
			...
			switch (eventData.moveDir)
			{
				...
				default:
					if (widgetState != WidgetState.Focussed)
						SelectDirection(eventData.moveDir);
					//{
		...
		{
			bool handled = false;
			if (parentList != null && parentList.SelectDirection(dir))
			{
				PlayAudio(MenuAudio.Move);
				handled = true;
			}
			...
			//	if (parentList != null && parentList.layout == MultiInputWidgetList.Layout.Vertical)
			//	{
			...
			//else
			//{
			//	if (parentList != null && parentList.layout == MultiInputWidgetList.Layout.Horizontal)
			//	{
			//		parentList.SelectDirection(dir);
			//		PlayAudio(MenuAudio.Move);
			//	}
